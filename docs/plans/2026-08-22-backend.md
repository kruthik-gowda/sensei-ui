...
def parse_diff(diff_text: str) -> List[Dict]:
    """Parse a SINGLE file's unified diff into rows. Returns [] for empty input.

    Multi-file input produces rows that cannot be attributed back to a file —
    callers pass one file's diff text at a time.
    """
    ...
    for patched_file in patch:
        for hunk in patched_file:
            ...
            for line in hunk:
                # The "\ No newline at end of file" marker has neither a source
                # nor a target line number and is neither an add nor a del.
                # Detect it structurally — its wording is not locale-stable.
                if (
                    not line.is_added
...
def delete_discussions(client, project_path: str, mr_iid: int, ids: List[str]) -> int:
    ...
    # Summary findings share one note, so ids can repeat; dedupe or the
    # second delete 404s against an already-removed note.
